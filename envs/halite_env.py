...
from hlt.networking import SocketGame

class HaliteIIEnv():
    def __init__(self, stdio=False, replay=True,
                 halite_binary_path="../halite", enemy_launch_cmd="python",
                 halite_opts=("-t", "-i", "replays"), socket_port="65214",
                 enemy_bot_path="../ElementaryBot.py"):
        ...
        # Game opts
        self.action_space = PLANET_MAX_NUM
        self.observation_space = np.zeros((PLANET_MAX_NUM, PER_PLANET_FEATURES))
        self.my_last_planets = 0
        self.my_last_ships = 0


    def step(self, actions):
        commands = self.__get_commands_from_actions(actions)
        print(f"[HE-STEP]: Commands: {commands}")
        self.game.send_command_queue(commands)
        self.game.map = self.game.update_map()
        done = self.game.done
        if done:
            print(f"[HE-STEP]: Game done!")
            self.close()
        self.start_round = time.time()
        reward = self.__calculate_rewards()
        print(f"[HE-STEP]: Reward: {reward}")
        observation = self.__get_observation(self.game.map)
    ...
    def reset(self):
        if self.stdio:
            self.game = hlt.Game(self.bot_name)
            self.process = sub.Popen(
                [self.halite_binary_path, *self.halite_opts,
                 f'python ../networking/socket_translator.py {self.socket_port}',
                 f"{self.enemy_launch_cmd} {self.enemy_bot_path}"])  # ../envs/SocketBot.py
        else:
            if self.replay:
                print("Soket: ", [self.halite_binary_path, *self.halite_opts, f'python ../networking/socket_translator.py {self.socket_port}', #
                     f"{self.enemy_launch_cmd} {self.enemy_bot_path}"])

                self.process = sub.Popen(
                    [self.halite_binary_path, *self.halite_opts, f'python ../networking/socket_translator.py {self.socket_port}',
                     f"{self.enemy_launch_cmd} {self.enemy_bot_path}"]) #../envs/SocketBot.py
            else:
                print("Soket: ", [self.halite_binary_path, *self.halite_opts, "replays",
                     f'python envs/SocketBot.py {self.socket_port}',
                     f'{self.enemy_launch_cmd} {self.enemy_bot_path}'])
                self.process = sub.Popen(
                    [self.halite_binary_path, *self.halite_opts, "replays",
                     ...
                     f'{self.enemy_launch_cmd} {self.enemy_bot_path}'],
                    stdout=sub.PIPE)
            self.game = SocketGame(self.bot_name, self.socket_port)
        self.my_last_ships = 0
        self.my_last_planets = 0
        self.game.map = self.game.update_map()
        return self.__get_observation(self.game.map)
    ...
    def __get_commands_from_actions(self, actions):
        # print(f'{action=}')

        total = np.sum(np.exp(actions), -1)
        # print(f'{total=}')
        predictions = np.exp(actions) / total
        # print(f'{predictions=}')

        ships_to_planets_assignment = self.__produce_ships_to_planets_assignment(map, predictions)
        commands = self.__produce_instructions(map, ships_to_planets_assignment)

        return commands

    def __calculate_rewards(self):
        reward = 0
        my_id = self.game.map.get_me().my_id

        my_planets = len([planet for planet in self.game.map.all_planets()
                          if planet.is_owned() and planet.owner.id == my_id])
        my_ships_number = len(self.game.map.friendly_ships())
        enemies_ships_number = len(self.game.map.enemies_ships())

        # weight planets profit
        if my_planets > self.my_last_planets:
            reward += 100 * (my_planets - self.my_last_planets)
        if my_planets < self.my_last_planets:
            reward -= 100 * (self.my_last_planets - my_planets)
        self.my_last_planets = my_planets

        # weight ships profit
        if my_ships_number > self.my_last_ships:
            reward += 10 * (my_ships_number - self.my_last_ships)
        if my_ships_number < enemies_ships_number:
            reward -= 5 * (enemies_ships_number - my_ships_number)
        self.my_last_ships = my_ships_number

        return reward


    def __get_observation(self, game_map):
        """
    ...
    def __produce_ships_to_planets_assignment(self, game_map, predictions, start_time, time_limit):
        """
    ...
    def __produce_instructions(self, game_map, ships_to_planets_assignment, round_start_time):
        """
            ...
            if is_planet_friendly:
                if ship.can_dock(planet):
                    ...
                    command_queue.append(
                        self.__navigate(game_map, round_start_time, ship, ship.closest_point_to(planet), speed))
            else:
                ...
                for s in docked_ships:
                    if weakest_ship is None or weakest_ship.health > s.health:
                        weakest_ship = s
                command_queue.append(
                    self.__navigate(game_map, round_start_time, ship, ship.closest_point_to(weakest_ship), speed))
        return command_queue


    def __navigate(self, game_map, start_of_round, ship, destination, speed):
        """
